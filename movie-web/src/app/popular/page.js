...
export default function Home() {
  ...
  const getData = async () => {
    ...
    setPopularMovieList(jsonData.results);
    setLoading(false);
  };
  useEffect(() => {
    getData();
  ...
  if (!loading && typeof PopularMovieList === undefined) {
        ...
        <div className="w-full flex justify-center mt-5">
          <div className="w-full flex text-black flex-col">
            <div className="w-full flex justify-between">
              <div className="w-60 ml-2 text-2xl font-semibold">Popular</div>
            </div>
            <div className="grid grid-cols-5 w-full mt-2 justify-around gap-3 max-sm:grid-cols-2  items-center px-3">
